...
class multimodalLLM:
    """ multimodalLLM에 관한 모음집 - GPT-4o 및 GPT-5 지원"""
    
    # 지원 모델 및 기본 설정
    SUPPORTED_MODELS = {
        "gpt-4o-mini": {"max_tokens": 16384, "supports_vision": True, "supports_video": True},
        "gpt-4o": {"max_tokens": 16384, "supports_vision": True, "supports_video": True},
        "gpt-5-mini": {"max_tokens": 128000, "supports_vision": True, "supports_video": True},
        "gpt-5": {"max_tokens": 128000, "supports_vision": True, "supports_video": True},
        "o1-preview": {"max_tokens": 32768, "supports_vision": False, "supports_video": False},
        "o1-mini": {"max_tokens": 65536, "supports_vision": False, "supports_video": False}
    }
    
    def __init__(self, llm_name: str = "gpt-4o", api_key: str = None):
        self.llm_name = llm_name
        
        # 모델 유효성 검사
        if llm_name not in self.SUPPORTED_MODELS:
            print(f"경고: {llm_name}은 지원되지 않는 모델입니다. 지원 모델: {list(self.SUPPORTED_MODELS.keys())}")
            print("gpt-5-mini로 기본 설정합니다.")
            self.llm_name = "gpt-5-mini"
        
        self.model_config = self.SUPPORTED_MODELS[self.llm_name]
        
        if "gpt" in self.llm_name or "o1" in self.llm_name:  # OpenAI 모델들
...
if __name__ == "__main__":
    ...
    # .env 파일에서 환경변수 로드
    load_dotenv()
    api_key = os.getenv('OPENAI_API_KEY')  # .env 또는 환경변수에서 API 키 읽기

    if not api_key:
        print("경고: OPENAI_API_KEY 환경변수가 설정되지 않았습니다.")
        print("루트 경로 또는 현재 경로의 .env 파일에 'OPENAI_API_KEY=your-api-key-here' 형식으로 추가하세요.")
        api_key = input("API 키를 직접 입력하세요: ")
    
    # 다양한 모델 테스트
    test_models = ["gpt-4o", "gpt-4o-mini", "gpt-5-mini"]
    
    for llm_name in test_models:
        print(f"\n=== {llm_name} 테스트 ===")
        try:
    ...
    # 실제 사용 예시 (주석 처리)
    # system_prompt = "You are an excellent video analyst. Help me with your abilities."
    # user_prompt = "비디오에서 어떤 행동을 하는지 단계적으로 설명하시오."
    # image_path = "your_video_file.mp4"  # 실제 파일 경로로 변경
    # answer = llm.query_answer_chatGPT(system_prompt, user_prompt, image_path)
    # print(answer)