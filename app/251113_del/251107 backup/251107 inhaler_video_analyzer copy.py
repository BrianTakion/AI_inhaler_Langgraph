    ...
    while start_time <= play_time - segment_time:
        ...
        try:
            ...
            for q_key, answer in current_q_answers.items():
                ...
                q_answers_accumulated[q_key].append((round(start_time, 1), answer))
...
# score에 따라 다른 점 스타일로 그리기 (score=1: 채워진 점, score=0: 빈 점)
action_times_filled = []
action_y_pos_filled = []
action_keys_filled = []
action_times_empty = []
...
# score=1인 action step 점들 추가 (채워진 점)
if action_times_filled:
    fig.add_trace(go.Scatter(
        ...
        showlegend=False,
        hovertemplate='%{text}<br>Time: %{x:.1f}s<br>Score: 1<extra></extra>',
        text=action_keys_filled
    ))

# score=0인 action step 점들 추가 (빈 점)
if action_times_empty:
    fig.add_trace(go.Scatter(
        ...
        showlegend=False,
        hovertemplate='%{text}<br>Time: %{x:.1f}s<br>Score: 0<extra></extra>',
